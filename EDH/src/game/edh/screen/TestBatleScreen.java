...
import game.edh.actors.batle.TestBatleGroup;
import game.edh.actors.games.HPBar;
import game.edh.frame.BaseScreen;

public class TestBatleScreen extends BaseScreen {
	...
	public TestBatleScreen(EdhGame game, float stageWidth, float stageHeight) {
		super(game, stageWidth, stageHeight);
		// TODO 自動生成されたコンストラクター・スタブ
		gGroup = new TestBatleGroup();
		mainTable.addActor(gGroup);

		TextureRegion image = Assets.batle.getRegion("batleback");
		...
		pauseButtun.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				// TODO 自動生成されたメソッド・スタブ
				if (TestBatleScreen.this.type != PAUSE) {
					pause();
					// isPause = true;
					// type = PAUSE;
					// gGroup.pause();
...
//			stage.act();
	...
	public void updateRunning(float delta) {
		...
		if (nowcombo != TestBatleGroup.combo) {
			nowcombo = TestBatleGroup.combo;
			if (nowcombo == 0)
				combo.setText("");
			else
