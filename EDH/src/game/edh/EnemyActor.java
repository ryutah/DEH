...
import game.edh.frame.EnemyEvent.AtackEvent;
...
public class EnemyActor extends Actor {
	...
	public EnemyActor() {
		...
		addListener(new InputListener() {
			...
			public boolean touchDown(InputEvent event, float x, float y,
				...
				if(AtackDeley < 0.2f) {
					nowHP -= Player.ATACK * 20.0f;
					GameTestGroup.addCombo();
				}
				else {
					nowHP -= Player.ATACK;
					GameTestGroup.combo = 0;
				}
				if (nowHP <= 0) {
					remove();
				}
	...
	public EnemyActor init(float scale) {
		this.scale = scale;
	...
	public void Atack() {
		...
		if(AtackDeley <= 0) {
			EnemyActor.this.fire(Pools.obtain(AtackEvent.class));
			setColor(Color.RED);
			//nowAtack = false;
			AtackDeley = 0.8f;
			stateTime = 0;
